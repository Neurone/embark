...
Blockchain.prototype.createFundAndUnlockAccounts = function(cb) {
  if (this.client.name === 'parity') {
    ...
    // 1) before starting the client (parityClient::initDevChain) we create the accounts and we append the passwords to the dev password file
    ...
    //TODO-#773 send funds
    console.log("this.config.unlockAddressList:" + this.config.unlockAddressList);
...
Blockchain.prototype.initDevChain = function(callback) {
  ...
  self.client.initDevChain('.embark/development/datadir', (err) => {
    ...
    // If requested create other accounts
    if (self.config.account && self.config.account.numAccounts) {
      async.waterfall([
