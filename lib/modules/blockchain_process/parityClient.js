...
class ParityClient {
  ...
  determineNetworkType(config) {
    ...
    /*
    --chain=[CHAIN]
        Specify the blockchain type. CHAIN may be either a JSON chain specification file or ethereum, classic, poacore,
        tobalaba, expanse, musicoin, ellaism, easthub, social, olympic, morden, ropsten, kovan, poasokol, testnet, or
        dev. (default: foundation)
    */

    if (config.networkType === 'rinkeby') {
      //TODO-#773 ERROR, parity cannot attach to rinkeby
    }

    //TODO-#773 Parity accepts JSON configuration as custom network
    //if(config.networkType === 'custom')

    let cmd = "--chain=" + config.networkType;
    ...
    if (config.networkType === 'testnet') {
      ...
      cmd = "--network-id=" + config.networkId;
    }
    return cmd;
  }
