...
class Whisper {
  ...
  connectToProvider() {
    ...
    // The best choice is to use void origin, BUT Geth rejects void origin, so to keep both clients happy we can use http://localhost:8000
  ...
  setServiceCheck() {
    ...
    self.events.request("services:register", 'Whisper', function(cb) {
      ...
      // TODO-#733 : Parity does not implement shh_version rpc method, use shh_info to check errors
  ...
  addSetProvider() {
    ...
    const consoleConfig = Object.assign({}, config, {providerOptions: {headers: {Origin: "http://localhost:8000"}}});
